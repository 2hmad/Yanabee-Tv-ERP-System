		...
		if (($handle = fopen('uploads/csv_file/student.generate.csv', 'r')) !== FALSE) { // Check the resource is valid
			...
			while (($all_data = fgetcsv($handle, 1000, ",")) !== FALSE) { // Check opening the file is OK!
				if($count > 0){
					...
					$user_data['gender'] = html_escape($all_data[4]);
					$user_data['nationality'] = html_escape($all_data[5]);
					$user_data['recitation'] = html_escape($all_data[6]);
					$user_data['conservation'] = html_escape($all_data[7]);
					$user_data['recitation_level'] = html_escape($all_data[8]);
					$user_data['day'] = html_escape($all_data[9]);
					$user_data['hour'] = html_escape($all_data[10]);
					$user_data['method'] = html_escape($all_data[11]);
					$user_data['confirm'] = html_escape($all_data[12]);
					$user_data['role'] = $role;
